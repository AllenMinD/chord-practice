...
- 8 个原有歌曲和弦案例保留在“现代和弦库”，构成公式与术语默认折叠
...
导航中的「读谱训练」提供 6 个可自由选择的阶段：地标音、高音谱表、低音与加线、音程形状、节奏敲击、连续音名识读。每组 10 题，8 题独立答对标记达标；提示题和错题可在结果页复习。最佳成绩保存在本机 localStorage，无法保存时仍可练习。

用 A–G 键输入音名，2–5 键输入音程度数，空格敲节奏。节奏关先点击「预备并开始」，按所选速度播放四拍（首拍重音）并同步显示数字；第四拍完整结束后才接受敲击，重练重新预备。切页、页面隐藏或静音会取消预备与敲击状态。节奏按所选 BPM 比较相邻起音间隔，容差为四分之一拍，不测最后一音的保持时长。连续读谱关测音名顺序，不测演奏节拍或 MIDI；本模块的短音型为原创识谱题，不是新增歌曲或歌曲播放器。

教学参考：[musictheory.net](https://www.musictheory.net/exercises) 的分项认音、音程训练，以及 [ABRSM](https://teacherhub.abrsm.org/mod/page/view.php?id=1831) 的提前看谱建议。阶段安排与达标阈值是本应用的教学设计，不是官方考试标准。
